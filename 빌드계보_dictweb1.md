...
- **판정 모드 margin (기본, 2026-08-28 사용자 확정 — 게이트를 판정에서 제외)**:
  ...
  통과 = 방출 토큰 신뢰도 conf ≥ λ(**기본 0.8** — 2026-08-28 사용자 실기기 보정, 표본 적음·
  파일럿 재보정 대상. 0.8에서 성인 골든의 약 절반이 재촬영 안내로 떨어짐) AND
  **판독 완전성 C ≥ 0.75** AND
  강제 정렬 여유도 M ≤ τ1. C(자유 판독 자모수/정답 자모수)는 3차 실기기 진단에서 발견된
  구멍(날려쓴 글씨가 일부 글자만 자신 있게 읽혀 conf·M을 통과: '나무'→'나' conf 0.72,
  M 0.165)을 막는다 — 실측: 날려쓴 실패 C 0.50~0.67 / 정자 1.00 / 성인 골든 33/36 ≥0.78
  (골든 3건은 모델이 실제로 절반만 읽은 크롭 → 재촬영 안내로 전환, 정답 인정 100%→91.7%
  트레이드오프 문서화).
...
- ~~통과 낱말 재촬영 유지~~ → **폐지(2026-08-28 사용자 확정)**: 매 촬영 전 칸을 새로
  판정한다 — 정자로 통과했다가 다시 날려 쓰면 현재 촬영 기준으로 미달 안내. 통과 여부는
  항상 최신 촬영 결과.
- 탈출구: 같은 word_id 미달(illegible/REJECT) 누적 3회 → 다음 촬영부터 override 통과("열심히 썼네요!").
