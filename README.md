...
통과 = **AI 읽기 신뢰도(conf ≥ λ) + 철자 검증(CTC 강제 정렬 여유도 M ≤ τ1)** —
인쇄체처럼 잘 읽히는 글씨는 통과, 삐뚤삐뚤/판독 불가 글씨는 "글자를 읽기 어려워요"로
바른 글씨를 유도한다(margin 모드 기본; 게이트 v1은 도메인 한계로 판정에서 제외하고
전 칸 로깅만 — `judge_mode` 설정으로 기존 게이트 판정 복원 가능). 자유 전사 비교는
쓰지 않는다(τ1/τ2/λ 설정 가능, 전 칸 M·conf 로깅). 통과한 낱말은 **라이브 카메라 AR**에서 학습지의 **그림 단서
...
- **성인 테스터 유의**: 게이트는 아동이 칸을 채우듯 크게(칸 높이 2/3 이상) 연필로 진하게
  쓴 글씨로 학습됨. 성인식 작은 글씨(칸 높이 절반 미만·연한 필압)는 깔끔해도 0등급으로
  거부된다 — 2026-08-27 iPhone 실기기 진단으로 확인(글씨 크기가 지배 요인, 진단자료 참고).
  문제가 재현되면 설정(⚙) → 「최근 판정 진단」에서 등급·M·크롭을 확인할 것.
